...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCentered) {
            longOffset += ((longSize - longPaddingBefore - longPaddingAfter) - ((count - 1)
                    * threeRadius + count * mRadius * 2)) / 2.0f;
        }
        ...
        for (int iLoop = 0; iLoop < count; iLoop++) {
            float drawLong = longOffset + (iLoop * threeRadius) + iLoop
                    * mRadius * 2;
            if (mOrientation == HORIZONTAL) {
                dX = drawLong;
                dY = shortOffset;
        ...
        float cx = currentPage * threeRadius + currentPage * mRadius * 2;
        if (!mSnap) {
            cx += mPageOffset * (mRadius * 2 + threeRadius);
        }
        if (mOrientation == HORIZONTAL) {
            dX = longOffset + cx;
            dY = shortOffset;
    ...
    public void onPageScrolled(int position, float positionOffset,
        ...
        if(mViewPagerScrollListener != null) {
            mViewPagerScrollListener.scrolling(positionOffsetPixels, position);
        }
    }
    ...
    private int measureLong(int measureSpec) {
        ...
        if ((specMode == MeasureSpec.EXACTLY) || (mViewPager == null)) {
            ...
            result = (int) (getPaddingLeft() + getPaddingRight()
                    + (count * 2 * mRadius) + (count - 1) * mRadius * mMargin + 1);
    ...
    public interface ViewPagerScrollListener {
        void scrolling(int positionOffsetPixels, int position);

        void scrollFinish(int position);
    }
