...
import com.e.common.widget.overscroll.ext.viewpager.OverViewPager;
 ...
 */
public class AutoScrollViewPager extends OverViewPager {
    ...
    private void setViewPagerScroller() {
        try {
            ...
            scrollerField.set(this.getOverScrollView(), scroller);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void scrollOnce() {
        PagerAdapter adapter = getOverScrollView().getAdapter();
        int currentItem = getOverScrollView().getCurrentItem();
        int totalCount;
        ...
        if (nextItem < 0) {
            if (isCycle) {
                getOverScrollView().setCurrentItem(totalCount - 1, isBorderAnimation);
            }
        } else if (nextItem == totalCount) {
            if (isCycle) {
                getOverScrollView().setCurrentItem(0, isBorderAnimation);
            }
        } else {
            getOverScrollView().setCurrentItem(nextItem, true);
        }
    }
        ...
        if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT || slideBorderMode == SLIDE_BORDER_MODE_CYCLE) {
            ...
            if ((currentItem == 0 && downX <= touchX) || (currentItem == pageCount - 1 && downX >= touchX)) {
                if (slideBorderMode != SLIDE_BORDER_MODE_TO_PARENT) {
                    if (pageCount > 1) {
                        getOverScrollView().setCurrentItem(pageCount - currentItem - 1, isBorderAnimation);
                    }
