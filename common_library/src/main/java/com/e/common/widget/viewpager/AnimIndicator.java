package com.e.common.widget.viewpager;
...
import com.e.common.widget.viewpager.adapter.RecyleAdapter;
...
public class AnimIndicator extends LinearLayout implements PageIndicator {
    ...
    private int mIndicatorSelectedBackground = R.drawable.selected_radius;
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            mIndicatorSelectedBackground = typedArray.getResourceId(R.styleable.AnimIndicator_ci_selected_drawable,
    ...
    public void onPageSelected(int position) {
        ...
        View preView = getRealChildAt(mCurrentPage);
        if (!CommonUtility.Utility.isNull(preView)) {
            preView.setBackgroundResource(mIndicatorBackground);
        }

        View currentView = getRealChildAt(position);
        if (!CommonUtility.Utility.isNull(currentView)) {
            currentView.setBackgroundResource(mIndicatorSelectedBackground);
        }
    ...
    private void createIndicators(ViewPager viewPager) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (i == 0) {
                indicator.setBackgroundResource(mIndicatorSelectedBackground);
            } else {
